...
import com.example.demo.data_tables.mapping.*;
...
public class DataTablesUtils {
    ...
    private static List<Criteria> getCriteria(final DataTablesInput input) {
        ...
        List<Column> orColumns = input.getOrColumns();
            ...
            if (column.hasValidSearch()) {
                ...
                if (filter != null) {
                    ...
                    if (StringUtils.hasLength(filter.getEq())) {
                        ...
                        if (StringUtils.hasLength(filter.getAll())) {
                            // $regex also works here
                            c.all(convertArray(type,filter.getAll()));
                            hasValidCrit = true;
                        }

                        if (filter.getNull() != null && filter.getNull().booleanValue()) {
                            c.is(null);
                            hasValidCrit = true;
                        }

                        if (filter.getEmpty() != null && filter.getEmpty().booleanValue()) {
                            c.is("");
                            hasValidCrit = true;
                        }
        ...
        if (!orColumns.isEmpty()) {
            ...
            for (final Column column : orColumns) {
                final Search search = column.getSearch();
                final ColumnType type = ColumnType.parse(column.getType());
                final Filter filter = column.getFilter();
                if (column.hasValidSearch()) {
                    ...
                    if(filter!=null && search.isRegex()){
                        if(StringUtils.hasLength(filter.getNin())){
                            c=new Criteria();
                            c.andOperator(Criteria.where(column.getData()).regex(getLikeFilterPattern(search.getValue())), Criteria.where(column.getData()).nin(convertArray(type, filter.getNin())));
                        }
                    }
                    ...
                    if (filter != null) {
                        ...
                        if (StringUtils.hasLength(filter.getEq())) {
                            ...
                            if (StringUtils.hasLength(filter.getNe()) && !StringUtils.hasLength(filter.getRegex())) {
                                // $ne
                                c.ne(type.tryConvert(filter.getNe()));
                                hasValidCrit = true;
                            }
                            ...
                            if (StringUtils.hasLength(filter.getNin()) && StringUtils.hasLength(filter.getRegex())) {
                                if (StringUtils.hasLength(filter.getNin())) {
                                    c=new Criteria();
                                    c.andOperator(Criteria.where(column.getData()).regex(filter.getRegex(), "i"), Criteria.where(column.getData()).nin(convertArray(type, filter.getNin())));
                                }else {
                                    c.regex(filter.getRegex(), "i");
                                }
                                hasValidCrit = true;

                            } else if (StringUtils.hasLength(filter.getNin())) {
                                c.nin(convertArray(type, filter.getNin()));
                                hasValidCrit = true;

                            } else if (StringUtils.hasLength(filter.getRegex())) {
                                c.regex(filter.getRegex(), "i");
                                hasValidCrit = true;
                            }
    ...
    public static Pageable getPageable(DataTablesInput input) {
        ...
        for (com.example.demo.data_tables.mapping.Order order : input.getOrder()) {
            Column column = null;
            if (StringUtils.hasLength(order.getData())) {
                ...
                if (order.getColumn() != null && input.getColumns() != null
                        && input.getColumns().size() > order.getColumn() && order.getColumn()>=0) {
    ...
    private static class DataTablesPageRequest implements Pageable {
        ...
        public DataTablesPageRequest(long offset, int pageSize, Sort sort) {
            this.offset = offset;
